...
class AnnotationDrivenBeanDefinitionParser implements BeanDefinitionParser {
	...
	private static class AopAutoProxyConfigurer {
		...
		public static void configureAutoProxyCreator(Element element, ParserContext parserContext) {
			// 注册InfrastructureAdvisorAutoProxyCreator的bean定义
			...
			if (!parserContext.getRegistry().containsBeanDefinition(txAdvisorBeanName)) {
				...
				// 创建TransactionAttributeSource的bean定义并注册
				...
				// 创建TransactionInterceptor的bean定义并注册
				...
				// 创建BeanFactoryTransactionAttributeSourceAdvisor的bean定义并注册
				...
				// 如果配置了order属性，则导入到bean属性中
				if (element.hasAttribute("order")) {
					advisorDef.getPropertyValues().add("order", element.getAttribute("order"));
				}
				parserContext.getRegistry().registerBeanDefinition(txAdvisorBeanName, advisorDef);

				// 创建CompositeComponentDefinition的bean并注册
