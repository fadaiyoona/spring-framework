...
public class ReflectiveAspectJAdvisorFactory extends AbstractAspectJAdvisorFactory implements Serializable {
	...
	public List<Advisor> getAdvisors(MetadataAwareAspectInstanceFactory aspectInstanceFactory) {
		...
		// 获取DeclareParents注解
		// DeclareParents主要用于引介增强的注解形式的实现，而其实现方式与普通增强很类似，只不过使用DeclareParentsAdvisor对功能进行封装，不深入解析
	...
	/**
	 * 获取切点，即获取指定注解的表达式信息，如@Before("test()")
	 * @param candidateAdviceMethod  切面类中的方法，候选方法
	...
	protected static class SyntheticInstantiationAdvisor extends DefaultPointcutAdvisor {
		...
		public SyntheticInstantiationAdvisor(final MetadataAwareAspectInstanceFactory aif) {
			// 注意第二个参数，lambda表达式，在目标方法调用前，又加了一个类似于@Before的增强器，用来做初始化工作
