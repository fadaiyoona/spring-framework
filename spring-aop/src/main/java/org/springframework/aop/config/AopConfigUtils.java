	...
	private static BeanDefinition registerOrEscalateApcAsRequired(
		...
		if (registry.containsBeanDefinition(AUTO_PROXY_CREATOR_BEAN_NAME)) {
			...
			if (!cls.getName().equals(apcDefinition.getBeanClassName())) {
				...
				// 的一个逻辑~~~~（就是防止怕用户注册错了，做了一个容错处理~~~）
				int currentPriority = findPriorityForClass(apcDefinition.getBeanClassName());
				int requiredPriority = findPriorityForClass(cls);
				// currentPriority < requiredPriority  如果当前用户注册进来的Aop代理类的级别，是低于我们要求的级别的，
				// Spring内部也会对它进行提升成我们要求的那个class类型
		...
		// 注册一个AnnotationAwareAspectJAutoProxyCreator，Bean的名称是“org.springframework.aop.config.internalAutoProxyCreator”
