...
public abstract class AbstractAutoProxyCreator extends ProxyProcessorSupport
	...
	/**
	 * 指定的interceptorNames是否优先调用，true-优先于其他增强器
	 */
	...
	/**
	 * 在postProcessBeforeInstantiation已经创建好代理的bean，用于在wrapIfNecessary中创建bean代理时，判断之前是否已经创建好代理了，避免重复创建
	 */
	private final Set<String> targetSourcedBeans = Collections.newSetFromMap(new ConcurrentHashMap<>(16));

	private final Map<Object, Object> earlyProxyReferences = new ConcurrentHashMap<>(16);

	// 已经创建了代理了的bean，把他对应的class缓存起来
	private final Map<Object, Class<?>> proxyTypes = new ConcurrentHashMap<>(16);

	/**
	 * 标明bean是否需要增强，value-true表示需要增强
	 */
	 ...
	 * @see #buildAdvisors
	 *
	 * 代理类的创建及处理，Spring委托给了ProxyFactory去处理，而在此函数中主要是对ProxyFactory的初始化操作，进而对真正的创建代理做准备，这些初始化操作包括如下内容：
	 * 1、获取当前类中的属性。
	 * 2、添加代理接口。
	 * 3、封装advisor并加入到ProxyFactory中。
	 * 4、设置要代理的类。
	 * 5、当然在Spring中还为子类提供了定制的customizeProxyFactory，子类可以在此函数中进行对ProxyFactory的进一步封装。
	 * 6、进行获取代理操作。
	 */
