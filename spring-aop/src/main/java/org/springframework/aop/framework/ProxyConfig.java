...
public class ProxyConfig implements Serializable {
	...
	// true：无论被代理的类是否接口，都使用CGLIB代理
	// false：接口->JDK动态代理，非接口->CGLIB动态代理
	private boolean proxyTargetClass = false;

	// 标记是否对代理进行优化。true：那么在生成代理对象之后，如果对代理配置进行了修改，已经创建的代理对象也不会获取修改之后的代理配置。如果有接口就代理接口(使用JDK动态代理)，没有接口代理类（CGLIB代理）
